...
export const getBreadCrumbList = (route:any, homeRoute:any) => {
  ...
  if (routeMetched.some((item:any) => item.name === homeRoute.name)) return [homeItem]
...
export const showTitle = (item:any, vm:any) => {
  ...
  if (!title) return
  if (useI18n) {
    if (title.includes('{{') && title.includes('}}') && useI18n) title = title.replace(/({{[\s\S]+?}})/, (m:any, str:any) => str.replace(/{{([\s\S]*)}}/, (m:any, _:any) => vm.$t(_.trim())))
 ...
 */
export const getHomeRoute = (routers:any, homeName:any = 'home'):any => {
  let i = -1
...
// scrollTop animation
// export const scrollTop = (el:any, from:any = 0, to:any, duration:any = 500, endCallback:any) => {
 ...
 */
export const setTitle = (routeItem:any, vm:any) => {
